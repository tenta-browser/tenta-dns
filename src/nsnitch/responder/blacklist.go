...
func checkblacklists(cfg *runtime.Config, rt *runtime.Runtime, ip net.IP) *BlacklistData {
	ret := &BlacklistData{Results: make(map[string]bool)}
	responses := make(chan *dnscheckresponse)
	started := 0
	for _, rbl := range cfg.Blacklists {
		go blacklistlookup(cfg, rt, ip, rbl, responses)
		started += 1
	}
	for started > 0 {
		select {
...
func blacklistlookup(cfg *runtime.Config, rt *runtime.Runtime, ip net.IP, rbl string, responder chan *dnscheckresponse) {
